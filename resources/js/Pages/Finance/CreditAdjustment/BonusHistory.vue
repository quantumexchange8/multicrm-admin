...
const getResults = async (page = 1, meta_login = props.account.meta_login, type = '', date = '') => {
    ...
    try {
        let url = `/finance/getBonusHistory/${meta_login}?page=${page}`;
...
const getAmountClass = (history) => {
    if (history.type === 'bonus_out') {
        return 'text-[#FF3F34]';
    } else if (history.type === 'bonus_in') {
        return 'text-[#05C46B]';
    }
    return '';
};
    ...
    <div class="grid grid-cols-3 mt-8 gap-6">
        <div class="space-y-2">
            ...
            >
                ...
                <option value="bonus_in">{{ $t('public.Bonus In') }}</option>
                <option value="bonus_out">{{ $t('public.Bonus Out') }}</option>
            </InputSelect>
        </div>
        <div class="space-y-2">
