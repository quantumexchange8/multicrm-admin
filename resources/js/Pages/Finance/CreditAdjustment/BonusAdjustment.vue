...
const submitForm = () => {
    form.post(route('finance.bonus_adjustment'), {
        onSuccess: () => {
            closeModal();
        },
    });
...
<template>
    <h2 class="text-lg mb-2 font-medium text-gray-900 dark:text-gray-100">{{ $t('public.Bonus Adjustment') }}</h2>
    <hr>
    ...
    <ul class="grid w-full gap-6 md:grid-cols-2 mt-8">
        <li>
            <input type="radio" id="bonus_in" v-model="form.type" value="bonus_in" @change="handleTypeChange('bonus_in')" class="hidden peer">
            <label for="bonus_in" class="inline-flex items-center justify-between text-center w-full py-2 text-white bg-white border border-blue-600 rounded-full cursor-pointer dark:hover:text-white dark:border-blue-600 dark:peer-checked:text-white dark:peer-checked:bg-[#007BFF] peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-blue-100 dark:text-white dark:bg-dark-eval-0 dark:hover:bg-blue-500">
                <span class="w-full">{{ $t('public.Add Bonus') }}</span>
            </label>
        </li>
        <li>
            <input type="radio" id="bonus_out" v-model="form.type" value="bonus_out" @change="handleTypeChange('bonus_out')" class="hidden peer">
            <label for="bonus_out" class="inline-flex items-center justify-between text-center w-full py-2 text-white bg-white border border-blue-600 rounded-full cursor-pointer dark:hover:text-white dark:border-blue-600 dark:peer-checked:text-white dark:peer-checked:bg-[#007BFF] peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-blue-100 dark:text-white dark:bg-dark-eval-0 dark:hover:bg-blue-500">
                <span class="w-full">{{ $t('public.Withdraw Bonus') }}</span>
            </label>
        </li>
    </ul>
    ...
    <form action="">
        <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-6">
            <div class="space-y-6 mb-4 md:mb-0">
                <Label for="source">{{ $t('public.Bonus Source') }}</Label>
                <Input
                    disabled
            ...
            <div class="space-y-6 mb-4 md:mb-0">
                <Label for="credit">{{ $t('public.Available Bonus Balance') }} ($)</Label>
                <Input
                    disabled
                    class="w-full border-0"
                    :model-value="account.bonus ?? 0.00"
                />
            </div>
            <div class="space-y-6 md:col-span-2 mb-4 md:mb-0" v-if="form.type === 'bonus_in'">
